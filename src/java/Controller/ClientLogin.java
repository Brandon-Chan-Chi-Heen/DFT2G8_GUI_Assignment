...
//                response.sendRedirect("/Client/welcome.jsp");
            } else {
                request.setAttribute("errorMessage", "Data not Found, Click on Register button.");
                request.getRequestDispatcher(request.getContextPath() + "/Client/LoginRegistration/login.jsp").forward(request, response);
            }
        } catch (Exception ex) {
